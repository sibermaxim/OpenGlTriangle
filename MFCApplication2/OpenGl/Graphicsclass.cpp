...
bool GraphicsClass::Initialize(COgl* OpenGL, HWND hwnd)
{
	...
	m_OpenGL = OpenGL;
	m_OpenGL->GetTranslWorldMatrix(tranMatr);

	tr = glm::vec3(-tranMatr[3][0], -tranMatr[3][1], -tranMatr[3][2]);
...
void GraphicsClass::MouseMove(CPoint point) 
{
	...
	m_OpenGL->SetTranslWorldMatrix(tmpMatrix);
	Render();
}
...
void GraphicsClass::Rotate(float deltaX, float deltaY)
{
	oldRotX += deltaX;
	oldRotY += deltaY;

	glm::mat4 m_rotateMatrix = glm::yawPitchRoll(glm::radians(oldRotX), glm::radians(oldRotX), 0.0f);
	m_OpenGL->SetRotatelWorldMatrix(m_rotateMatrix);
